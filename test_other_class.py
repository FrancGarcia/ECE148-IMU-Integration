...
class ArtemisOpenLog:
    def __init__(self, port, baudrate, timeout):
        ...
        self.gyro_x_smooth = deque(maxlen=100)
        self.gyro_y_smooth = deque(maxlen=100)
        self.gyro_z_smooth = deque(maxlen=100)
        try:
...
def log_data():
    try:
        def update_plot(_):
            artemis_imu.poll()
            line_gx_n.set_data(range(len(artemis_imu.gyro_x)), list(artemis_imu.gyro_x))
            line_gy_n.set_data(range(len(artemis_imu.gyro_y)), list(artemis_imu.gyro_y))
            line_gz_n.set_data(range(len(artemis_imu.gyro_z)), list(artemis_imu.gyro_z))

            line_gx_s.set_data(range(len(artemis_imu.gyro_x_smooth)), list(artemis_imu.gyro_x_smooth))
            line_gy_s.set_data(range(len(artemis_imu.gyro_y_smooth)), list(artemis_imu.gyro_y_smooth))
            line_gz_s.set_data(range(len(artemis_imu.gyro_z_smooth)), list(artemis_imu.gyro_z_smooth))
            

            ax.set_xlim(0, len(artemis_imu.gyro_x))
            return line_gx_n, line_gy_n, line_gz_n

        fig_noisy, ax = plt.subplots()
        ax.set_xlabel("Time")
        ax.set_ylabel("Noisy Gyroscope (deg/s)")
        ax.set_ylim(-10, 10)
        line_gx_n, = ax.plot([], [], label="gX", color="red")
        line_gy_n, = ax.plot([], [], label="gY", color="green")
        line_gz_n, = ax.plot([], [], label="gZ", color="blue")
        line_gx_s, = ax.plot([], [], label="gX", color="black")
        line_gy_s, = ax.plot([], [], label="gY", color="orange")
        line_gz_s, = ax.plot([], [], label="gZ", color="pink")
        ax.legend()

        ani = animation.FuncAnimation(fig_noisy, update_plot, interval=5, blit=True, cache_frame_data=False)
        plt.show()
    except KeyboardInterrupt:
